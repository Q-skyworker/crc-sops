...
def data(request, biz_cc_id):
    ...
    ctx = task.get_act_data(act_id, component_code, subprocess_stack)
    return JsonResponse(ctx)
...
def detail(request, biz_cc_id):
    ...
    subprocess_stack = json.loads(request.GET.get('subprocess_stack', '[]'))
    task = TaskFlowInstance.objects.get(pk=task_id, business__cc_id=biz_cc_id)
    ctx = task.get_act_detail(act_id, component_code, subprocess_stack)
    return JsonResponse(ctx)


@require_POST
@check_user_perm_of_task(['execute_task'])
...
@require_POST
@check_user_perm_of_task(['fill_params'])
