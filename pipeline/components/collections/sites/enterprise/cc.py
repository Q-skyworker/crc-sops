...
class CCUpdateHostService(Service):
    ...
    def execute(self, data, parent_data):
        ...
        if cc_host_property == "bk_isp_name":
            bk_isp_name = cc_format_prop_data(executor, 'host', 'bk_isp_name', parent_data.get_one_of_inputs('language'))
            ...
            bk_state_name = cc_format_prop_data(executor, 'host', 'bk_state_name', parent_data.get_one_of_inputs('language'))
            ...
            bk_province_name = cc_format_prop_data(executor, 'host', 'bk_province_name', parent_data.get_one_of_inputs('language'))
...
class CCReplaceFaultMachineService(Service):
    ...
    def execute(self, data, parent_data):
        ...
        for item in cc_hosts:
            clone_kwargs.update({"bk_org_ip": item['cc_fault_ip'].strip()})
            clone_kwargs.update({"bk_dst_ip": item['cc_new_ip'].strip()})
            # 克隆主机属性会自动把主机移动到对应模块
            clone_result = client.cc.clone_host_property(clone_kwargs)
            if not clone_result['result']:
                message = cc_handle_api_error('cc.clone_host_property', clone_kwargs, clone_result['message'])
                data.set_outputs('ex_data', message)
                return False

            fault_machine.append(item['cc_fault_ip'].strip())

        # 将故障机上交至故障机模块
        host_result = cc_get_host_id_by_innerip(executor, biz_cc_id, fault_machine)
        if not host_result['result']:
            data.set_outputs('ex_data', host_result['message'])
            return False
        ...
        cc_result = client.cc.transfer_host_to_faultmodule(to_fault_module_kwargs)
        if cc_result['result']:
            return True
        else:
...
class CCUpdateSetService(Service):
    ...
    def execute(self, data, parent_data):
        ...
        if cc_set_property == "bk_service_status":
            bk_service_status = cc_format_prop_data(executor, 'set', 'bk_service_status', parent_data.get_one_of_inputs('language'))
...
class CCUpdateModuleService(Service):
    ...
    def execute(self, data, parent_data):
        ...
        if cc_module_property == "bk_module_type":
            bk_module_type = cc_format_prop_data(executor, 'module', 'bk_module_type', parent_data.get_one_of_inputs('language'))
